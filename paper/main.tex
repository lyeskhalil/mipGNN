...
\begin{algorithm}[H]\mbox{\hfill}
	...
	\begin{algorithmic}[1]
		...
		\For{$t \text{ in } [T]$}
		%\State \textit{Compute variable embedding:}
...
Proof idea: show that GNN can be seen as low "degree" (assuming low nnz) compositional functions encoded as a tree, then use results on composiotional function (smaller set of parameters) to derive generalization bounds

\cm{Somehow show that architecture of Alg. 2 (MWU) is better than using a standard GNN. Idea: Show that MWU can be recovered to show to always get at ferasible solutions.}
