...
\begin{algorithm}[H]\mbox{\hfill}
	\\\textbf{Input:}  Training set $S$, number of epochs $E$, number of layers $T$, stepsize $\eta > 0$.  \\
	\textbf{Output:} A trained model $f_{\boldsymbol{\theta}} \colon \cI \to [0,1]^n$.
	\begin{algorithmic}[1]
		...
		\For{$t \text{ in } [T]$}
		\State $\vec{V}_j^{(t)} \leftarrow {f}^{(t)}_{\text{V}} \hspace{-1pt} \big( \big\{ \hspace{-4.4pt} \big\{ \big( \vec{V}^{(t-1)}_j, \vec{C}^{(t-1)}_j, \vec{p}^{(t-1)}_j, \vec{A}_{ij} \big)\hspace{-1pt} \mid c_i \in N(v_j) \big\} \hspace{-4.4pt} \big\} \big)$ for all $j$ in $[n]$ 		
		
		\State $\vec{X}_j^{(t)} \leftarrow {f}^{(t)}_{\text{X}} \hspace{-2pt} \big(\vec{V}_i^{(t-1)} \big)$ for all $j$ in $[n]$ 
		\Statex
		\Statex
		\State $\vec{C}_j^{\hspace{1pt}(t)} \leftarrow {f}^{(t)}_{\text{C}} \hspace{-2pt} \big( \big\{ \hspace{-4.4pt} \big\{ \big( \vec{C}^{(t-1)}_j, \vec{V}^{(t)}_i, \vec{X}^{(t)}_i, \vec{A}_{ij}, \vec{b}_j \big)\hspace{-2pt} \mid v_i \in N(c_j) \big\} \hspace{-4.4pt} \big\} \big)$ for all $i$ in $[m]$ 	
		%\State $\vec{e}_j^{(t)} \leftarrow \sum_{i \in N(j)} \label{key}\vec{A}_{ji}  \vec{x}_i - \vec{b}_j$
		%\Statex
		\State $\vec{p}_{j}^{(t)} \hspace{3pt}\leftarrow \text{softmax}(f_{\text{P}}(\vec{e}^{(t)}))_{j}$
		\EndFor
		\State $\bar{\vec{x}} \leftarrow f(\mathbin\Vert_{t \in [T]} \vec{x}^{(t)})$
		\State $L \leftarrow L + \ell(\vec{\bar{x}}, \vec{\bar b})$ 
		\State Gradient descent on $L$ with regard to $\boldsymbol{\theta}$
		\EndFor
		\State \Return $\boldsymbol{\theta}$
